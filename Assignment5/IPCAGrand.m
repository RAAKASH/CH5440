...
%Finds the error variances for all posssible values for the number of contraints                      
% Input :  C - Data matrix (feature x no of points)
%                          (  Row   x    Column   )                      
% Depends -stdest.m
%---------------------------------Code----------------------------------------------------------
[m,n]=size(C);
...
No = j;
    for i = 1:100
     L = SigmaE.^0.5;     % L matrix | L'*L = SigmaE
     D = L\(C);
     [U,S,V] = svd(D,'econ');
